...
document.addEventListener('DOMContentLoaded', () => {
    // Configurar el formulario para agregar comentarios
    document.getElementById('comment-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        const taskId = document.getElementById('comment-task-id').value; // ID de la tarea
        const comment = document.getElementById('task-comment').value; // Nuevo comentario
        
        await addComment(taskId, comment); // Llamar a la función para agregar comentario
        document.getElementById('task-comment').value = ''; // Limpiar el campo de texto
        showComments(taskId); // Actualizar la lista de comentarios
    });

    // Abrir el modal y cargar comentarios al hacer clic en una tarea
    const taskElements = document.querySelectorAll('.task-item'); // Selector de tareas en tu vista
    taskElements.forEach(task => {
        task.addEventListener('click', async () => {
            const taskId = task.dataset.taskId; // Asegúrate de que cada tarea tenga un atributo data-task-id
            document.getElementById('comment-task-id').value = taskId; // Establecer el ID de la tarea en el modal
            await showComments(taskId); // Mostrar los comentarios en el modal
            const modal = new bootstrap.Modal(document.getElementById('commentModal'));
